...
@property (nonatomic, assign) NSInteger sessionTimeout DEPRECATED_MSG_ATTRIBUTE("Sessions are now handled by Liquid backend (server-side).");
...
- (NSString *)sessionIdentifier DEPRECATED_MSG_ATTRIBUTE("Sessions are now handled by Liquid backend (server-side).");
